...
In the first phase, participants knew exactly what outcome they wanted: given a target execution shown as a captioned video, they had to revise the current plan to reproduce it, and we evaluate \oursystem{} against a baseline without plan context.
...
Both conditions ran on the same interface: participants could replay the simulated execution of the current plan at any time, and the baseline differed only in that the Gantt timeline and task-reference functionality were hidden.

\paragraph{Autonomous Planning Baseline.}

The baseline used the same underlying planning and execution pipeline as our system, but hid the Gantt timeline visualization and plan-reference functionality.
...
Consequently, when participants wanted to revise an existing plan, they needed to describe a complete revised task specification that encoded both the requested change and the existing task decisions that should be preserved.
...
The experimenter then briefly introduced both system conditions, after which participants completed a short, unscored practice exercise with each system to become familiar with its basic interaction workflow.

\paragraph{Phase 1 (40 min)}
In the first phase, we compared our plan-contextualized co-authoring interface with an autonomous planning baseline.
...
Each participant worked in the scenario they had completed under the \emph{baseline} condition in Phase 1.
...
At the beginning of each scenario, only the initial plan was loaded into the assigned interface; the underlying prompts used to generate the saved plans were never shown to participants.
...
% - Scenario = the participant's Phase-1 BASELINE scenario (counterbalanced, so half sorting /
%   half breakfast): familiar task semantics, but full system is fresh in it; enables
%   spontaneous baseline-vs-full-system comparisons in think-aloud.
...
% 1. I could use language to express the task changes I intended.
% 2. Selecting objects, locations, or plan tasks as references made it easier to express
%    what I meant.
%
% Direct manipulation of the timeline:
% 3. The timeline made it easy to understand task assignments and ordering.
% 4. Dragging tasks on the timeline made it easy to reorder or reassign them.
%
% Direct manipulation in the scene:
% 5. The scene controls made it easy to adjust robot routes.
% 6. The scene controls made it easy to adjust object placements.
%
% - Think-aloud + interview themes: which revisions users prefer to say vs. drag, and why;
%   do NOT frame interview questions as baseline comparisons (demand characteristics).
