...
What the user receives is the execution outcome, not the plan itself: which robot was assigned which task, in what order, and under which cross-robot dependencies remains internal to the planner.
Even when such systems surface these decisions---as inter-agent dialogue~\cite{chen2024scalable} or dependency-annotated task decompositions~\cite{wang2024dartllm}---the plan is presented as output to be accepted or regenerated, not as a persistent artifact the user can directly act upon.
